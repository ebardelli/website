...
With help from GitHub Copilot, I rewrote the backend to be more reliable and flexible about where it pulls news from. The new version is live on [Ed News](https://ebardelli.com/ed-news/). It was a fun, low-stakes project and a good way to practice [*vibe coding*](https://x.com/karpathy/status/1886192184808149383): LLM-assisted, exploratory, low-pressure coding where the focus is on learning and tinkering rather than perfection. LLM assistance is great for rapid iteration, but verify generated code and outputs before trusting them in production.
...
The article recommendation engine is one of the features made possible with this rewrite. Technically, it's quite simple. It just uses a single SQL query over [Nomic AI's pre-computed embeddings](https://www.nomic.ai/blog/posts/nomic-embed-text-v1) to find semantically related articles. Under the hood, it uses [sqlite_vec](https://github.com/asg017/sqlite-vec) to interact with the vector embeddings. SQLite does a lot more with plain old SQL than you'd expect.

Specifically, the recommendation query computes cosine similarity between article embeddings (via sqlite_vec) to return nearest neighbors; results depend on the embedding model, normalization, and dataset size.

## Access to the Data

I also decided to share the item database through Parquet files so that the publication data is explorable. Parquet's columnar format works well with DuckDB, so you can query the files remotely without pulling everything down first. The two main databases are:
...
- Move the database from the local `SQLite` file to an online service (to enable multi-user access and automated updates).
