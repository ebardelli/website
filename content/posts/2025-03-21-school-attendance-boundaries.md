...
A few lessons from this project that may help others doing similar work.

Data and algorithms need human context. Our optimization models efficiently assigned census blocks to schools, but they couldn't account for historical neighborhood connections, walking routes, or community identities. The best boundaries came from blending algorithmic suggestions with local knowledge.

Flexible, reusable tools accelerate decision-making. By building our analysis on SQL-based DuckDB rather than point-and-click GIS software, we could respond quickly to new scenarios as they emerged during board meetings and community forums.

Census blocks are a good foundation. Using pre-defined blocks saved substantial time and gave us a neutral, recognized geographic unit that stakeholders could understand and reference.

Visualization matters. Our web tools converted complex geospatial data into maps that allowed non-technical stakeholders to actively participate in the boundary-drawing process.

## Tools and Technologies

**DuckDB** - Open-source analytical database used for geospatial analysis and census block assignments. Particularly useful for its SQL interface and spatial extension.
...
**WebAssembly (WASM)** - Allowed us to run DuckDB directly in the browser for the boundary drawing tool, providing immediate feedback on boundary change impacts.
